...
- Use the root control-plane folders as the source of truth: `architecture/`, `context/`, `decisions/`, `features/`, `memory/`, `plans/`, `prompts/`, and `workflows/`.
...
- Do not create a parallel `docs/` project brain or a repo-local `.amistio/` folder.
...
- Use `plans/` for non-trivial work plans.
- Use `decisions/` for accepted or proposed architectural/product choices.
- Use `features/` for product behavior specs.
- Use `memory/` only for durable lessons, mistakes, product rules, or conventions that future agents should reuse.
- Use `prompts/` for reusable or generated implementation prompts.
...
- When creating a new repo from this template, update stale template names and assumptions in `README.md`, `AGENTS.md`, `.github/copilot-instructions.md`, `context/`, and `architecture/` before implementation.
